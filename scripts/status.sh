...
opt_list=`ls /tmp/ | grep Sep`
for f in ${opt_list}
do
  margin=`echo $f | sed 's/^.*sp-\([0-9\.]*\)-.*$/\1/'`
  alg_str=`echo $f | sed 's/^.*speed_\([a-zA-Z\-]*\)_.*$/\1/'`
  obj_str=`cat /tmp/$f/maximize-speed_obj.dat | tail -n 1`
  eq_str=`cat /tmp/$f/maximize-speed_eq.dat | tail -n 1`
  ieq_str=`cat /tmp/$f/maximize-speed_ieq.dat | tail -n 1`
...
#cat /tmp/Feb-22-16-37-05-2019_forehand_maximize-speed_CCSA_48.0h_M-14_N-5_x-max-1.0_x-hit-0.5_maxvel-1_minjerk-0.0005_delta-1.7e-04_eqthre-1.0e-08_ftol-1.0e-15_xtol-1.0e-10_interval-20_sp-0-0-0-100-50_root-joint_modify-ec
